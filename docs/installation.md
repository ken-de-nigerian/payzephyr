...
Paystack is used here as an example because it's the default provider out of the box, but you can use any supported provider instead; see [Configuration](configuration.md) for every provider's required keys, and the [provider matrix](providers.md#subscription-and-refund-support-at-a-glance) for the current list and what each one supports.
