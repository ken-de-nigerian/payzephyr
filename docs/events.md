...
**`SubscriptionPaymentFailed`** — a recurring charge attempt failed (an expired card, insufficient funds). This does *not* necessarily mean the subscription is cancelled — most providers retry a few times before giving up, depending on their own dunning configuration:
